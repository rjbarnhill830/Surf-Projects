  ...
  sessionCache.forEach(s=>{
    ...
    row.innerHTML=`
      <div>
        ...
        ${s.forecast ? `<div class="meta" style="margin-top:3px;">Forecast at log time (${forecastSourceLabel(s.forecast)}): ${s.forecast.reading.swellH}ft @ ${s.forecast.reading.swellP}s ${dirLabelDeg(s.forecast.reading.swellDir)} &middot; ${energyLabel(s.forecast.reading.swellH,s.forecast.reading.swellP)}${s.forecast.reading.windS!=null?`, wind ${s.forecast.reading.windS}mph ${dirLabel(s.forecast.reading.windDir)}`:''} &mdash; vs. logged actual above</div>` : ''}
      </div>
